...
tidy.cva.glmnet <- function(x, extract = c("cvm", "beta"), ...){
  # estimate := cvm
  # 
  ...
  n <- length(alpha)
   ...
   for (i in 1:n) {
       ...
       dti <- switch (extract,
                 cvm  = tidy(modi),            # tidy cv.glmnet object
                 beta = tidy(modi$glmnet.fit)  ## tidy glmnet object
                     )
       dti$alpha <- alpha[i]
       dtlist[[i]] <- dti
     }
     ret <- dplyr::bind_rows(dtlist) %>% 
        relocate(alpha) %>% relocate (lambda,.after = alpha) 
...
dtlist <- vector("list", length = n )
for (i in 1:n) {
  modi_cv <- modlist[[i]] # cv.glmnet object
  
  dti_cv <- tidy(modi_cv)    # tidy cv.glmnet object
  dti_cv$alpha <- alphv[i]
  dtlist[[i]] <- dti
}

cva_res <- dplyr::bind_rows(dtlist) %>% arrange(estimate)
names(cva_res)
cva_res
#cva_res %>% print(n=5000)
