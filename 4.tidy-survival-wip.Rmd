...
```
This is work in progress (Dec.2022)
...
surv_recp_init <-
 ...
 surv_recp_init   
```
include recipe into a workflow. 

```{r surv-wf-recp}
 surv_wf_recp <- workflow()%>%  
     add_recipe(surv_recp_init)
 class(surv_wf_recp)
 surv_wf_recp
```
...
```{r model-formula}
mod_formula <- Surv(futime, status) ~ . 
```
## Model fit
...
class(surv_fit) # "_coxnet"   "model_fit"
glance(surv_fit)
tidy(surv_fit)
```
...
```{r surv-pred-surv}
pred_survival <- predict(surv_fit, data_test_init, penalty =0.6, 
                         type= "survival", time = c(3,6,9,12))
```

```{r surv-pred-time}
pred_time  <- predict(surv_fit, data_test_init, penalty = 0.6, type= "time")
Surv_obs <- Surv(data_test_init$futime, data_test_init$status)
cbind(Surv_obs, pred_time) # compare 
```

## Cross-validation (known alpa)
